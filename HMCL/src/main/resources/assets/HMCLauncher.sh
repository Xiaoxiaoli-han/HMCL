...
if [ -n "${HMCL_JAVA_HOME+x}" ]; then
  if [ -x "$HMCL_JAVA_HOME/bin/$_HMCL_JAVA_EXE_NAME" ]; then
    ...
    if [ "$_HMCL_USE_CHINESE" == true ]; then
      ...
      echo "你可以访问 https://afdian.net/group/e91474ceaac911ee8d1d52540025c377 页面寻求帮助。" 1>&2
    else
      echo "The value of the environment variable HMCL_JAVA_HOME is invalid, please set it to a valid Java path." 1>&2
      echo "You can visit the https://afdian.net/group/e91474ceaac911ee8d1d52540025c377 page for help." 1>&2
    fi
    exit 1
  fi
fi
...
if [[ "$_HMCL_ARCH" == "loongarch64" ]]; then
  if [ "$_HMCL_USE_CHINESE" == true ]; then
    echo "运行 PMCL 需要 Java 运行时环境，请安装龙芯 JDK (https://pmcl.fun/downloads/loongnix.html) 并设置环境变量后重试。" 1>&2
    echo "你可以访问 https://afdian.net/group/e91474ceaac911ee8d1d52540025c377 页面寻求帮助。" 1>&2
  else
    echo "The Java runtime environment is required to run HMCL." 1>&2
    echo "Please install Loongson JDK (https://pmcl.fun/downloads/loongnix.html) and set the environment variables, then try again." 1>&2
    echo "You can visit the https://afdian.net/group/e91474ceaac911ee8d1d52540025c377 page for help." 1>&2
  fi
  exit 1
fi
